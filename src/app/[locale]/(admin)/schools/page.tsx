...
import { Plus, School, Trash2, Edit } from 'lucide-react';
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white">{t('admin.schools')}</h1>
        <motion.button
      ...
      {showForm && (
        ...
        >
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="mb-1.5 block text-sm text-gray-400">Address</label>
              <input
                value={form.address}
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={loading} className="rounded-xl bg-violet-600 px-6 py-2 text-sm font-medium text-white hover:bg-violet-500 transition disabled:opacity-60">
              {t('common.save')}
            </button>
            <button type="button" onClick={() => setShowForm(false)} className="rounded-xl border border-white/10 px-6 py-2 text-sm font-medium text-gray-400 hover:text-white transition">
              {t('common.cancel')}
            </button>
          </div>
        </motion.form>
