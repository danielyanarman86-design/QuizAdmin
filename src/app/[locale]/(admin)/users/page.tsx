...
export default function UsersPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="mb-6 text-2xl font-bold text-white">{t('users')}</h1>

      <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm">
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/10">
              {[tCommon('user'), 'Email', tCommon('role'), tCommon('school'), tCommon('status'), tCommon('actions')].map((h) => (
                  ...
                  <div className="flex gap-1">
                    {user.status === 'pending' && (
                      <>
                        <button onClick={() => updateStatus(user.id, 'active')}
                          className="rounded-lg p-1.5 text-emerald-400 hover:bg-emerald-400/10 transition" title={t('approveTeacher')}>
                          <CheckCircle className="h-4 w-4" />
                        </button>
                        <button onClick={() => updateStatus(user.id, 'rejected')}
                          className="rounded-lg p-1.5 text-red-400 hover:bg-red-400/10 transition" title={t('rejectTeacher')}>
                    ...
                    {user.status === 'active' && (
                      <button onClick={() => updateStatus(user.id, 'banned')}
                        className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-400/10 transition" title={t('banUser')}>
