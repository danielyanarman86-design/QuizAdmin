...
export default function AdminQuizzesPage() {
  ...
  const fetchQuizzes = () => {
    api.get('/admin/quizzes').then((res) => setQuizzes(res.data)).catch(() => {}).finally(() => setLoading(false));
  };

  useEffect(() => { fetchQuizzes(); }, []);

  const handleDelete = async (id: string) => {
    if (!confirm(t('deleteQuizConfirm'))) return;
    ...
    <div className="p-6">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-gray-400">{t('noQuizzesYet')}</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                {[tCommon('title'), tCommon('category'), tCommon('author'), tCommon('questions'), tCommon('status'), tCommon('public'), tCommon('actions')].map((h) => (
            ...
            <tbody>
              {quizzes.map((quiz, i) => (
                <motion.tr key={quiz.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs font-medium ${quiz.isPublic ? 'text-emerald-400' : 'text-gray-500'}`}>
                      {quiz.isPublic ? tCommon('yes') : tCommon('no')}
                    </span>
                  </td>
                  <td className="px-4 py-3">
