...
export default function AdminStatisticsPage() {
  ...
  const cards = stats ? [
    ...
    { icon: TrendingUp, label: t('totalAttempts'), value: stats.totalAttempts, color: 'from-indigo-600 to-blue-600' },
    { icon: TrendingUp, label: t('avgScore'), value: `${stats.avgScore}%`, color: 'from-amber-600 to-yellow-600' },
  ] : [];
    ...
    <div className="p-6">
      ...
      {loading ? (
        <div className="flex gap-2 justify-center py-20">
          {[0, 1, 2].map((i) => <div key={i} className="h-2 w-2 rounded-full bg-rose-500 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />)}
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {cards.map((card, i) => (
              <motion.div key={card.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                ...
                <motion.p initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: i * 0.08 + 0.2, type: 'spring' }}
          ...
          {stats && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                <h3 className="mb-4 font-semibold text-white">{t('usersByRole')}</h3>
                <div className="space-y-3">
                  {[
                    { label: t('teachers'), value: stats.byRole.teachers, color: 'bg-violet-500' },
                    { label: t('students'), value: stats.byRole.students, color: 'bg-blue-500' },
                  ].map((item) => {
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                <h3 className="mb-4 font-semibold text-white">{t('usersByStatus')}</h3>
                <div className="space-y-3">
                  {[
                    { label: tCommon('active'), value: stats.byStatus.active, color: 'bg-emerald-500' },
                    { label: tCommon('pending'), value: stats.byStatus.pending, color: 'bg-yellow-500' },
                    { label: tCommon('banned'), value: stats.byStatus.banned, color: 'bg-red-500' },
                  ].map((item) => {
